...
def trigger(device_data, enable, source=trigger_source.none, channel=1, timeout=0, edge_rising=True, level=0, trigtype=0, horizontalpos=0):
    """
        set up triggering
        ...
        parameters: - device data
                    ...
                    - trigger type - possible: 0 means edge, 1 means pulse, 2 means transistion, 3 means window
                    - trigger horizontal position in seconds, default is 0s
    """
    if enable and source != constants.trigsrcNone:
        ...
        # set trigger type
        if dwf.FDwfAnalogInTriggerTypeSet(device_data.handle, ctypes.c_int(trigtype)) == 0:
            check_error()
        ...
        # set trigger horizontal position
        if dwf.FDwfAnalogInTriggerPositionSet(device_data.handle, ctypes.c_double(horizontalpos)) == 0:
            check_error()
    else:
